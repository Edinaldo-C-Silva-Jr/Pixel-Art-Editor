namespace PixelArtEditor.Grids
{
    ...
    internal class CheckerGrid : IGridGenerator
    {
        ...
        public void ApplyGridFullImage(Bitmap imageWithGrid)
        {
            ...
            for (int y = 0; y < imageWithGrid.Height / CheckerGridPiece.Height + 1; y++) // Iterates the amount of times needed to cover the full image horizontally
            {
                for (int x = 0; x < imageWithGrid.Width / CheckerGridPiece.Width + 1; x++) // Iterates the amount of times needed to cover the full image vertically
                {
